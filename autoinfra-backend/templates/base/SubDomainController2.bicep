...
param parentDomainControllerPrivateIp string = '' // Parent DC's private IP (immediate parent)
param rootDomainControllerPrivateIp string = '' // Root DC's private IP (for forest operations when parent != root)
param parentVirtualMachineHostname string = ''
...
// Skip peering flags - used during Update mode when peerings already exist from locked nodes
param skipParentPeering bool = false
param skipRootPeering bool = false
...
var parentDomainControllerFQDN = !empty(parentVirtualMachineHostname) && !empty(parentDomainName) ? '${parentVirtualMachineHostname}.${parentDomainName}' : ''
...
var rootNetwork = startsWith(rootDomainControllerPrivateIp, '10.10.') ? '10' : (startsWith(rootDomainControllerPrivateIp, '172.16.') ? '172' : '192')
var requiresPeering = !empty(parentDomainControllerPrivateIp) && currentNetwork != parentNetwork && !skipParentPeering
var requiresRootPeering = !empty(rootDomainControllerPrivateIp) && rootDomainControllerPrivateIp != parentDomainControllerPrivateIp && currentNetwork != rootNetwork && !skipRootPeering
...
var rootDCRules = requiresRootPeering ? [
  {
    name: 'Allow-RootDC-Communication-Inbound'
    properties: {
      description: 'Allows inbound traffic from root DC for forest operations'
      protocol: '*'
      sourcePortRange: '*'
      destinationPortRange: '*'
  ...
  {
    name: 'Allow-RootDC-Communication-Outbound'
    properties: {
      description: 'Allows outbound traffic to root DC for forest operations'
      protocol: '*'
      sourcePortRange: '*'
      destinationPortRange: '*'
      sourceAddressPrefix: privateIPAddress
...
var allNSG = concat(dcPeeringRules, rootDCRules, jumpboxRules, publicIPRules)
...
resource networkInterface 'Microsoft.Network/networkInterfaces@2022-11-01' = {
  ...
  properties: {
    dnsSettings:{
      dnsServers: !empty(rootDomainControllerPrivateIp) && rootDomainControllerPrivateIp != parentDomainControllerPrivateIp ? [
...
resource peeringCurrentToRoot 'Microsoft.Network/virtualNetworks/virtualNetworkPeerings@2023-05-01' = if (requiresRootPeering) {
  name: 'vnet-${currentNetwork}/peer-to-${rootNetwork}'
  properties: {
    allowVirtualNetworkAccess: true
    allowForwardedTraffic: true
    allowGatewayTransit: true
    useRemoteGateways: false
    remoteVirtualNetwork: {
      id: resourceId('Microsoft.Network/virtualNetworks', 'vnet-${rootNetwork}')
    }
  }
  dependsOn: [networkInterface]
}

resource peeringRootToCurrent 'Microsoft.Network/virtualNetworks/virtualNetworkPeerings@2023-05-01' = if (requiresRootPeering) {
  name: 'vnet-${rootNetwork}/peer-to-${currentNetwork}'
  properties: {
    allowVirtualNetworkAccess: true
    allowForwardedTraffic: true
    allowGatewayTransit: true
  ...
  dependsOn: [networkInterface, peeringCurrentToRoot]
}
  ...
  properties: {
    parameters:[
      ...
      }
      {
        ...
        value: parentDomainControllerFQDN
      }
    ]
    source: {
